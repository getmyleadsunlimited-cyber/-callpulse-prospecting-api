...
An internal administrator bootstraps an account owner through `POST /users` by supplying `account_id`, `account_type`, and `primary_workspace_id`; subsequent owner-created memberships inherit those account fields. User administration is available at `GET/POST /users`, `PATCH /users/{membership_id}/role`, `POST /users/{membership_id}/deactivate`, and `PUT /users/{membership_id}/workspace-access`. The API rejects demotion or deactivation of an account's last active owner. Creation, role change, deactivation, session revocation, and workspace-grant replacement write audit records.
