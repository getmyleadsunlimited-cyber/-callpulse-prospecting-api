...
Customer authentication identities are global by normalized email and may have separate memberships in multiple accounts. Each membership owns its account-specific role and workspace grants; login therefore requires `account_id`. Passwords are 12–256 characters and are stored only as salted PBKDF2-SHA256 hashes. PostgreSQL atomically reserves account/email and source rate-limit capacity before any fixed-cost password verification, so concurrent requests cannot exceed the 15-minute limits; telemetry stores only hashed keys and never passwords. Login returns a random opaque bearer token whose SHA-256 digest is persisted, expires after 12 hours by default (`CALLPULSE_SESSION_HOURS`), and is bound to one membership and the identity's security version.

`POST /auth/logout` revokes the current session, `POST /auth/revoke-all` revokes every identity session, `POST /auth/revoke-account` lets an owner revoke every session in the account, and an owner may revoke one membership's sessions with `POST /users/{membership_id}/revoke-sessions`. `POST /auth/change-password` verifies the current password, increments the security version, and immediately revokes every prior session. Deactivation revokes that membership's sessions; role and grant changes remain effective on the next request because authorization is loaded from the database rather than copied into the token.
...
An internal administrator bootstraps an account owner invitation through `POST /users` by supplying `account_id`, `account_type`, and `primary_workspace_id`; subsequent owner-created invitations inherit those account fields. Invitations are account-local, expire after 24 hours, and persist only a digest of a random one-time acceptance token. The inviter never supplies credentials and receives the same generic response whether the email is new or already global. A deployment must connect the `deliver_invitation_token` provider seam to an approved out-of-band delivery mechanism; invitation creation fails closed with `503` when it is not configured. A new identity establishes its password at `POST /invitations/accept`; an existing identity authenticates before accepting. User administration is available at `GET/POST /users`, `PATCH /users/{membership_id}/role`, `POST /users/{membership_id}/deactivate`, and `PUT /users/{membership_id}/workspace-access`. The API rejects demotion or deactivation of an account's last active owner. Invitation, acceptance, role change, deactivation, session revocation, and workspace-grant replacement write audit records.

Every workspace has an authoritative owning account and type. Direct/client memberships may use only an owned primary workspace. Agency memberships may additionally use client workspaces explicitly delegated to their agency account; internal administrators provision that relationship with `POST /internal/agency-workspaces`. A grant is accepted only when this ownership/delegation relationship exists. `X-Workspace-ID` remains only a selector: foreign selection is `403`, malformed selection is `400`, and resource lookup remains scoped first and returns `404` for foreign IDs. Legacy bearer grants and the separate internal-admin credential remain unchanged.
