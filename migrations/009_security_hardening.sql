...
CREATE TABLE IF NOT EXISTS legacy_tenancy_quarantine (
    id SERIAL PRIMARY KEY,
...
-- An agency relationship is authoritative only when the registry identifies a
-- client owner and the legacy agency grant explicitly names that workspace.
INSERT INTO agency_workspace_access(agency_account_id, workspace_id)
SELECT DISTINCT u.account_id, a.workspace_id
FROM user_workspace_access a
JOIN users u ON u.id = a.user_id AND u.account_type = 'agency'
JOIN workspace_ownership_registry r ON r.workspace_id = a.workspace_id
JOIN accounts owner_account ON owner_account.id = r.owner_account_id
WHERE r.owner_account_id <> u.account_id
  AND owner_account.account_type = 'client'
  AND r.workspace_type = 'client'
...
-- Preserve invalid historical claims for operator review, but never turn them
-- into memberships. A non-owner agency primary is valid only with the explicit
-- client-workspace delegation established above.
INSERT INTO legacy_tenancy_quarantine
    (legacy_user_id, account_id, workspace_id, record_type, reason)
SELECT u.id, u.account_id, u.primary_workspace_id, 'membership',
       'primary workspace is neither account-owned nor explicitly delegated to this agency'
FROM users u
LEFT JOIN workspace_ownership_registry r ON r.workspace_id = u.primary_workspace_id
LEFT JOIN agency_workspace_access d
       ON d.agency_account_id = u.account_id AND d.workspace_id = u.primary_workspace_id
WHERE u.account_id IS NOT NULL
  AND (r.workspace_id IS NULL OR (r.owner_account_id <> u.account_id AND d.id IS NULL));

INSERT INTO account_memberships(user_id, account_id, primary_workspace_id, role, active)
SELECT u.id, u.account_id, u.primary_workspace_id, u.role, u.active
FROM users u
JOIN workspace_ownership_registry r ON r.workspace_id = u.primary_workspace_id
LEFT JOIN agency_workspace_access d
       ON d.agency_account_id = u.account_id AND d.workspace_id = u.primary_workspace_id
WHERE u.account_id IS NOT NULL
  AND (r.owner_account_id = u.account_id OR d.id IS NOT NULL)
...
-- Quarantine every grant that lacks an authoritative workspace, a migrated
-- membership, account ownership, or an explicit agency/client delegation.
INSERT INTO legacy_tenancy_quarantine
    (legacy_user_id, account_id, workspace_id, record_type, reason)
SELECT u.id, u.account_id, a.workspace_id, 'workspace_grant',
       'workspace grant is not authorized by ownership or agency delegation'
FROM user_workspace_access a
JOIN users u ON u.id = a.user_id
LEFT JOIN workspace_ownership_registry r ON r.workspace_id = a.workspace_id
LEFT JOIN account_memberships m ON m.user_id = u.id AND m.account_id = u.account_id
LEFT JOIN agency_workspace_access d
       ON d.agency_account_id = u.account_id AND d.workspace_id = a.workspace_id
WHERE r.workspace_id IS NULL OR m.id IS NULL
   OR (r.owner_account_id <> u.account_id AND d.id IS NULL);

INSERT INTO membership_workspace_access(membership_id, workspace_id)
SELECT DISTINCT m.id, a.workspace_id
FROM user_workspace_access a
JOIN users u ON u.id = a.user_id
JOIN account_memberships m ON m.user_id = u.id AND m.account_id = u.account_id
JOIN workspace_ownership_registry r ON r.workspace_id = a.workspace_id
LEFT JOIN agency_workspace_access d
       ON d.agency_account_id = u.account_id AND d.workspace_id = a.workspace_id
WHERE r.owner_account_id = u.account_id OR d.id IS NOT NULL
